...
const OrganisateurRegisterForm = () => {
  ...
  const RegisterRequest = async (data: Object) => {
    await axios.post('http://localhost:5000/api/organisateur/register', data)
    .then(res => {
      console.log(res);
    })
  }
  ...
  let [image, setImage] = React.useState<File | any>(
                ...
                <div className="mt-6 flex flex-col">
                  ...
                  <input
                    ...
                    {...register("image_profile", {
                      onChange: (e: React.ChangeEvent<HTMLInputElement>) => e.target.files && setImage(URL.createObjectURL(e.target.files[0])),
                    })}
                  />
                </div>
                <div className="flex items-center border-2 py-2 px-3 rounded-md mt-4">
